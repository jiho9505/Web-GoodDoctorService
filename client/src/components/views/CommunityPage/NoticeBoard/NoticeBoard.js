  ...
  const columns = [
    ...
    const renderBoard = () => {
      ...
      if(props.isLoading){
        ...
        return <div style={{display:'flex', justifyContent:'center', alignItems:'center', paddingBottom:'30px'}}>
                <Spin indicator={antIcon} />
               </div>
      }else{
        if(props.list){
          if(props.list.length>0){
            return <Table dataSource={Datas} columns={columns} pagination={{defaultPagesize : 15, Pagesize: 15}}  size='small' bordered={true} />
          }else{
            return <div>
                      <Empty/>
                      <br/><br/>
